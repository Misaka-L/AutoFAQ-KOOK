...
namespace AutoFAQ.Core.Services
{
    public class CommandHandleService : IHostedService
    {
        ...
        public CommandHandleService(ILogger<CommandHandleService> logger, CommandService commandService,
            KookSocketClient socketClient, IServiceProvider serviceProvider)
        {
            _logger = logger;
        ...
        private async Task _commandService_CommandExecuted(CommandInfo commandInfo, ICommandContext context,
            IResult result)
        {
            ...
            {
                await context.Channel.SendCardsAsync(
                    new CommandErrorCardMessage(commandInfo, context, result).Build());
            }
        }

        private async Task _socketClient_MessageReceived(SocketMessage arg, SocketUser socketGuildUser,
            IMessageChannel arg3)
        {
